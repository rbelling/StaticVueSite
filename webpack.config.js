...
const devConfig = {
  ...
  entry: [path.join(__dirname, 'src', 'javascript', 'page.js')],
  output: {
    path: PATHS.dist,
  ...
  plugins: [
    ...
    new StyleExtHtmlWebpackPlugin({
      //this lets us inline the critical css code
      minify: true
    })
  ],
  module: {
    loaders: [
      ...
      {
        test: /\.ttf(\?v=\d+\.\d+\.\d+)?$/,
        loader: `url?limit=10000&mimetype=application/octet-stream&name=${assetsFolder}/[name].[ext]`
      },
      { test: /\.ico$/, loader: 'file?name=[name].[ext]' },
      {
        test: /\.(jpe?g|png|gif|svg)$/i,
        loaders: [`url?limit=30000&name=${assetsFolder}/[name].[ext]`, 'img?progressive=true']
      },
      {
        test: /(\.css|\.scss|\.sass)$/,
        loaders: [
      ...
      },
      {
        test: /critical.scss$/,
        loaders: [
          StyleExtHtmlWebpackPlugin.inline(),
          'postcss',
          'resolve-url',
          'sass?sourceMap',
          'sass-resources'
